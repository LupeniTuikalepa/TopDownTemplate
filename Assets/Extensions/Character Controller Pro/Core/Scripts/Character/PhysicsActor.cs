...
namespace Lightbug.CharacterControllerPro.Core
{
    ...
    public abstract class PhysicsActor : MonoBehaviour
    {
        [Header("Rigidbody")]
        [Tooltip(
            "Interpolates the Transform component associated with this actor during Update calls. This is a custom implementation, the actor " +
        ...
        [Tooltip("Whether or not to use continuous collision detection (rigidbody property). " +
                 "This won't affect character vs static obstacles interactions, but it will affect character vs dynamic " +
                 "rigidbodies.")]
        public bool useContinuousCollisionDetection = true;

        [Header("Root motion")]
        [Tooltip("This option activates root motion for the character. With root motion enabled, position and rotation are " +
                 "handled exclusively by the animation system.")]
        ...
        [Condition(
            ...
            new ConditionAttribute.ConditionType[]
                {ConditionAttribute.ConditionType.IsTrue, ConditionAttribute.ConditionType.IsTrue},
            new float[] {0f, 0f},
        ...
        {
            get => Is2D ? Rotation * Vector3.right : Rotation * Vector3.forward;
            set
            {
        ...
        {
            get => Is2D ? Rotation * Vector3.forward : Rotation * Vector3.right;
            set
            {
        ...
        /// <param name="multiplier">It affects how much rotation is applied. This value goes from 0 (no rotation) to 1 (instant).</param> 
        public virtual void SetYaw(Vector3 forward, float multiplier = 1f)
        {
            forward = Vector3.ProjectOnPlane(forward, Up);
            float targetAngle = Vector3.SignedAngle(Forward, forward, Up);
            targetAngle *= Mathf.Clamp01(multiplier);
            Rotation = Quaternion.AngleAxis(targetAngle, Up) * Rotation;
        }
        ...
        /// <param name="forward">The desired forward vector.</param>
        /// <param name="multiplier">It affects how much rotation is applied. This value goes from 0 (no rotation) to 1 (instant).</param> 
        public virtual void SetPitch(Vector3 forward, float multiplier = 1f)
        {
            forward = Vector3.ProjectOnPlane(forward, Right);
            float targetAngle = Vector3.SignedAngle(Forward, forward, Right);
            targetAngle *= Mathf.Clamp01(multiplier);
            Rotation = Quaternion.AngleAxis(targetAngle, Right) * Rotation;
        }
