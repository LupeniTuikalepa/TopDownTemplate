...
namespace Lightbug.CharacterControllerPro.Core
{
    ...
    public class CharacterActor : PhysicsActor
    {
        ...
        [Tooltip("Objects with the OneWayPlatform component can be detected as one way platforms. In addition to that, " +
                 "characters can also detect one way platforms using a mask.")]
        public bool filterByLayerMask = true;

        [Condition("filterByLayerMask", ConditionAttribute.ConditionType.IsTrue, ConditionAttribute.VisibilityType.NotEditable)]
        public LayerMask oneWayPlatformsLayerMask = 0;

        [Tooltip(
            "This value defines (in degrees) the total arc used by the one way platform detection algorithm (using the bottom part of the capsule). " +
        ...
        [Header("Stability")]
        [Tooltip(
            "If the character is stable, the ground slope angle must be less than or equal to this value in order to remain \"stable\". " +
        ...
        [Tooltip(
            "A high planar velocity value (e.g. tight platformer) can be projected onto an unstable surface, causing the character to " +
        ...
        [Tooltip(
            "This will prevent the character from stepping over an unstable surface (a \"bad\" step). This requires a bit more processing, so if your character does not need this level of precision " +
            "you can disable it.")]
        ...
        [Tooltip(
            "The distance the character is capable of detecting ground. Use this value to clamp (or not) the character to the ground.")]
        [Min(0f)]
        public float stepDownDistance = 0.5f;


        [Header("Grounding")] [Tooltip("Prevents the character from enter grounded state (IsGrounded will be false)")]
        public bool alwaysNotGrounded = false;

        [Condition("alwaysNotGrounded", ConditionAttribute.ConditionType.IsFalse)]
        [Tooltip(
            "If enabled the character will do an initial ground check (at \"Start\"). If the test fails the starting state will be \"Not grounded\".")]
        public bool forceGroundedAtStart = true;

        [Tooltip(
            "This option will enable a trigger (located at the capsule bottom center) that can be used to generate OnTriggerXXX messages. " +
        ...
        [Tooltip(
            "With this enabled the character bottom sphere (capsule) will be simulated as a cylinder. This works only when the character is standing on an edge.")]
        ...
        [Header("Dynamic ground")] [Tooltip("Should the character be affected by the movement of the ground?")]
        ...
        [Tooltip(
            "The forward direction of the character will be affected by the rotation of the ground (only yaw motion allowed).")]
        public bool rotateForwardDirection = true;

        [Condition("supportDynamicGround", ConditionAttribute.ConditionType.IsTrue,
            ConditionAttribute.VisibilityType.NotEditable)]
        [Tooltip(
            "This is the maximum ground velocity delta (from the previous frame to the current one) tolerated by the character." +
        ...
        [Condition("supportDynamicGround", ConditionAttribute.ConditionType.IsTrue,
            ConditionAttribute.VisibilityType.NotEditable)]
        [Tooltip(
            "When the character becomes \"not grounded\" (after a ForceNotGrounded call) part of the ground velocity can be transferred to its own velocity. " +
        ...
        [Header("Velocity")] [Tooltip("Whether or not to project the initial velocity (stable) onto walls.")] [SerializeField]
        public bool slideOnWalls = true;

        [SerializeField] bool resetVelocityOnTeleport = true;
        ...
        [Header("Rotation")] [Tooltip("Should this component define the character \"Up\" direction?")]
        ...
        [Condition(
            ...
            new ConditionAttribute.ConditionType[]
                {ConditionAttribute.ConditionType.IsTrue, ConditionAttribute.ConditionType.IsNotNull},
            new float[] {0f, 0f},
        ...
        [Condition(
            new string[] {"constraintRotation", "upDirectionReference"},
            new ConditionAttribute.ConditionType[]
                {ConditionAttribute.ConditionType.IsTrue, ConditionAttribute.ConditionType.IsNull},
            new float[] {0f, 0f},
            ConditionAttribute.VisibilityType.Hidden)]
        ...
        [Header("Physics")] public bool canPushDynamicRigidbodies = true;
        ...
        public LayerMask ObstaclesLayerMask => PhysicsComponent.CollisionLayerMask;
        ...
        public Quaternion GroundRotation =>
            Is2D ? Quaternion.Euler(0f, 0f, GroundRigidbody2D.rotation) : GroundRigidbody3D.rotation;

        /// <summary>
        /// Returns true if the current ground is a Rigidbody (2D or 3D), false otherwise.
        /// </summary>
        public bool IsGroundARigidbody =>
            Is2D ? characterCollisionInfo.groundRigidbody2D != null : characterCollisionInfo.groundRigidbody3D != null;

        /// <summary>
        /// Returns true if the current ground is a kinematic Rigidbody (2D or 3D), false otherwise.
        /// </summary>
        public bool IsGroundAKinematicRigidbody => Is2D
            ? characterCollisionInfo.groundRigidbody2D.bodyType == RigidbodyType2D.Kinematic
            : characterCollisionInfo.groundRigidbody3D.isKinematic;
        ...
        public Vector3 GetGroundPointVelocity(Vector3 point)
        {
            ...
            return Is2D
                ? (Vector3) characterCollisionInfo.groundRigidbody2D.GetPointVelocity(point)
                : characterCollisionInfo.groundRigidbody3D.GetPointVelocity(point);
        }
        ...
        public string GetCharacterInfo()
        {
            ...
            return string.Concat(
                ...
                "Object Name : ",
                characterCollisionInfo.groundObject != null ? characterCollisionInfo.groundObject.name : NULL_STRING, '\n',
                "Layer : ", LayerMask.LayerToName(characterCollisionInfo.groundLayer), '\n',
                "Rigidbody Type: ",
                GroundRigidbodyComponent != null ? GroundRigidbodyComponent.IsKinematic ? "Kinematic" : "Dynamic" : NULL_STRING,
                '\n',
        ...
        {
            get { return CustomUtilities.ProjectOnTangent(Velocity, GroundStableNormal, Up); }
            set { Velocity = CustomUtilities.ProjectOnTangent(value, GroundStableNormal, Up); }
        }
        ...
        {
            get { return GetTopCenter(Position); }
        }
        ...
        {
            get { return GetBottomCenter(Position, 0f); }
        }
        ...
        {
            get { return GetBottomCenter(Position, StepOffset); }
        }
        ...
        HitFilterDelegate _collisionWithoutOWPHitFilter;
        ...
        protected override void Awake()
        {
            ...
            _collisionWithoutOWPHitFilter = CollisionWithoutOWPHitFilter;
        }
        ...
        protected override void Start()
        {
            ...
            // Initial OWP check
            var filter = new HitInfoFilter(ObstaclesLayerMask, false, true);
        ...
        protected virtual void ApplyWeight(Vector3 contactPoint)
        {
            ...
            if (Is2D)
            {
                ...
                GroundCollider2D.attachedRigidbody.AddForceAtPosition(-Up * (CharacterBody.Mass * weightGravity), contactPoint);
            }
            else
            {
        ...
        void HandleRotation()
        {
            ...
            if (upDirectionReference != null)
            {
                ...
                Vector3 referenceToTarget =
                    Is2D
                        ? Vector3.ProjectOnPlane(upDirectionReference.position - targetPosition, Vector3.forward)
                        : upDirectionReference.position - targetPosition;
                referenceToTarget.Normalize();

                constraintUpDirection = sign * referenceToTarget;
            }
        ...
        protected bool RemovePenetration(ref Vector3 position)
        {
            ...
            do
            {
                iterationOverlapDetected =
                    ColliderComponent.ComputePenetration(ref position, ref rotation, _removePenetrationAction);
        ...
        void RemovePenetrationAction(ref Vector3 bodyPosition, ref Quaternion bodyRotation, Transform otherColliderTransform,
            Vector3 penetrationDirection, float penetrationDistance)
        {
            ...
            if (CheckOneWayPlatformLayerMask(otherColliderTransform.gameObject))
            {
        ...
        /// </summary>
        public bool IsGroundAscending =>
            transform.InverseTransformVectorUnscaled(Vector3.Project(GroundVelocity * Time.deltaTime, Up)).y > 0;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal HitInfoFilter GetDefaultCheckSizeFilter() => new HitInfoFilter(ObstaclesLayerMask, false, true);
        ...
        /// <returns>True if the overlap doesn't detect any obstacle.</returns>
        public bool CheckAndSetSize(Vector3 position, Vector2 size, in HitInfoFilter filter,
        ...
        /// <param name="sizeReferenceType">Anchor point used when changing the size.</param>
        /// <returns>True if the overlap doesn't detect any obstacle.</returns>
        public bool CheckAndSetSize(Vector3 position, Vector2 size,
            SizeReferenceType sizeReferenceType = SizeReferenceType.Bottom)
        {
            var filter = GetDefaultCheckSizeFilter();
        ...
        /// <param name="filter"></param>
        /// <returns>True if the overlap doesn't detect any obstacle.</returns>
        public bool CheckAndSetSize(Vector2 size, in HitInfoFilter filter,
            SizeReferenceType sizeReferenceType = SizeReferenceType.Bottom) =>
        ...
        /// <param name="sizeReferenceType">Anchor point used when changing the size.</param>
        /// <returns>True if the overlap doesn't detect any obstacle.</returns>
        public bool CheckAndInterpolateSize(Vector2 targetSize, float lerpFactor, in HitInfoFilter filter,
            SizeReferenceType sizeReferenceType = SizeReferenceType.Bottom)
        {
        ...
        /// <param name="sizeReferenceType">Anchor point used when changing the size.</param>
        /// <returns>True if the overlap doesn't detect any obstacle.</returns>
        public bool CheckAndInterpolateSize(Vector2 targetSize, float lerpFactor,
            SizeReferenceType sizeReferenceType = SizeReferenceType.Bottom)
        {
            var filter = GetDefaultCheckSizeFilter();
        ...
        /// <returns>True if the overlap doesn't detect any obstacle.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool CheckAndInterpolateHeight(float targetHeight, float lerpFactor, in HitInfoFilter filter,
            SizeReferenceType sizeReferenceType = SizeReferenceType.Bottom) =>
        ...
        {
            HitInfoFilter filter = new HitInfoFilter(ObstaclesLayerMask, false, true);
            SweepAndTeleport(destination, in filter);
        }

        /// <summary>
        /// Sweeps the body from its current position (CharacterActor.Position) towards a desired destination. 
        /// Triggers and one way platforms will be ignored.
        /// </summary>
        ...
        public Vector3 GetGroundSlopeNormal(CollisionInfo collisionInfo)
        {
            ...
            if (collisionInfo.isAnEdge)
            {
                ...
                if (contactSlopeAngle < slopeLimit && collisionInfo.edgeUpperSlopeAngle <= slopeLimit &&
                    collisionInfo.edgeLowerSlopeAngle <= slopeLimit)
                    return Up;
        ...
        void ProbeGround(float dt)
        {
            ...
            HitInfoFilter overlapFilter = new HitInfoFilter(ObstaclesLayerMask, false, true);
            ...
            if (CheckOneWayPlatformLayerMask(collisionInfo.hitInfo.transform.gameObject))
            {
        ...
        bool CollisionHitFilter(Transform hitTransform)
        {
            ...
            // If it is a one way platform, ignore the collision. Do NOT ignore the hit, we need it!
            if (CheckOneWayPlatformLayerMask(go))
            {
                PhysicsComponent.IgnoreCollision(go.transform, true);
            }
            else // If not, filter collisions based on the object "get-ignore" value.
            {
        ...
        bool CollisionWithoutOWPHitFilter(Transform hitTransform)
        {
            var go = hitTransform.gameObject;

            // If it is a one way platform, ignore the collision AND the hit
            if (CheckOneWayPlatformLayerMask(go))
            {
                PhysicsComponent.IgnoreCollision(go.transform, true);
                return false;
                    ...
                    if (canPushDynamicRigidbodies)
                    {
                        if (collisionInfo.hitInfo.IsRigidbody)
                        {
                            if (collisionInfo.hitInfo.IsDynamicRigidbody)
                            {
                                ...
                                {
                                    bool canPushThisObject = CustomUtilities.BelongsToLayerMask(collisionInfo.hitInfo.layer,
        ...
        public bool IsGroundAOneWayPlatform
        {
            get
            {
                ...
                return CheckOneWayPlatformLayerMask(GroundObject);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool CheckOneWayPlatformLayerMask(GameObject other) =>
            other.TryGetComponent(out OneWayPlatform _) | 
            (filterByLayerMask && other.layer.BelongsToLayerMask(oneWayPlatformsLayerMask));
            ...
            while (iteration < CharacterConstants.MaxSlideIterations || displacement == Vector3.zero)
            {
                ...
                if (collisionInfo.hitInfo.hit)
                {
                    ...
                    if (collisionInfo.hitInfo.IsRigidbody)
                    {
                        ...
                        {
                            bool canPushThisObject =
                                CustomUtilities.BelongsToLayerMask(collisionInfo.hitInfo.layer, pushableRigidbodyLayerMask);
                            if (canPushThisObject)
                            {
                                position += displacement;
                                break;
                    ...
                    if (slidePlaneANormal == Vector3.zero)
                    {
                        if (preventUnstableClimbing && bottomCollision && !stableHit)
                        {
                            ...
                            bool isUpwardsDisplacement =
                                transform.InverseTransformVectorUnscaled(Vector3.Project(projectedMovement, Up)).y > 0f;
        ...
        void ProcessNewGround(Transform newGroundTransform)
        {
            ...
            if (isThisANewGround)
            {
                ...
                groundRigidbodyComponent =
                    groundRigidbodyComponents.GetOrRegisterValue<Transform, RigidbodyComponent>(newGroundTransform);
                OnNewGroundEnter?.Invoke();
            }
        }

        bool UpdateCollideAndSlideData(CollisionInfo collisionInfo, ref Vector3 slidingPlaneNormal, ref Vector3 groundPlaneNormal,
