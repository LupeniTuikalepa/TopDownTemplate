...
namespace Lightbug.CharacterControllerPro.Demo
{
    ...
    {
        [Min(0f)] [SerializeField] protected float initialVelocity = 12f;

        [Min(0f)] [SerializeField] protected float duration = 0.4f;

        [SerializeField] protected AnimationCurve movementCurve = AnimationCurve.Linear(0, 1, 1, 0);

        [Min(0f)] [SerializeField] protected int availableNotGroundedDashes = 1;

        [SerializeField] protected bool ignoreSpeedMultipliers = false;

        [SerializeField] protected bool forceNotGrounded = true;

        [Tooltip("Should the dash stop when we hit an obstacle (wall collision)?")] [SerializeField]
        ...
        public override void EnterBehaviour(float dt, CharacterState fromState)
        {
            ...
            if (CharacterActor.IsGrounded)
            {
                ...
                {
                    currentSpeedMultiplier = materialController != null
