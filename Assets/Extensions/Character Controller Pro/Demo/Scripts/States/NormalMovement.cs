...
namespace Lightbug.CharacterControllerPro.Demo
{
    ...
    public class NormalMovement : CharacterState
    {
        ...
        protected virtual void ProcessPlanarMovement(float dt)
        {
            ...
            bool needToAccelerate = (CharacterStateController.InputMovementReference * currentPlanarSpeedLimit).sqrMagnitude >= 
            ...
            switch (CharacterActor.CurrentState)
            {
                case CharacterActorState.NotGrounded:
                    ...
                    targetPlanarVelocity = CharacterStateController.InputMovementReference * (speedMultiplier * currentPlanarSpeedLimit);
        ...
        void HandleLookingDirection(float dt)
        {
            ...
            // The second parameter is a lerp factor multiplier
            // If the character is 2D, do not interpolate yaw rotation (factor = 1).
            // If the character is 3D, use the looking direction "speed" value as angular velocity.
            CharacterActor.SetYaw(targetLookingDirection, CharacterActor.Is2D ? 1f : lookingDirectionParameters.speed * dt);
        }
        ...
        void SetTargetLookingDirection(LookingDirectionParameters.LookingDirectionMovementSource lookingDirectionMode)
        {
            ...
            else
            {
                if (CharacterActor.PlanarVelocity != Vector3.zero)
                    targetLookingDirection = Vector3.ProjectOnPlane(CharacterActor.PlanarVelocity, CharacterActor.Up).normalized;
                else
                    targetLookingDirection = CharacterActor.Forward;
            }
        }
